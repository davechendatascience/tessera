...
Step 3 (MVP) of the Feynman improvement plan: detect multiplicative
separability via log-log linear regression. If R² > threshold, build
a seed tree `C · ∏ x_i^{a_i}` and inject into the GP initial
population.

Hypothesis: the search-cost of independently rediscovering the
exponent structure is what bottlenecks ~10 of the 30 Feynman
equations (Coulomb, Larmor, Stokes-Einstein, ideal gas, ...). Seeding
the GP with the right power-law skeleton lets const-opt do the rest.

Pass criteria (§6 discipline):
  - At least 5 partial→exact OR failed→partial transitions on
    detected power-law targets.
  - 0 regressions on currently-exact equations (the seed competes
    with the existing exact form via selection; a regression would
    indicate a bug in the injection mechanism).
...
def main(argv=None):
    ...
    print(f"=== Feynman decompose A/B (n_gens={args.n_gens}, pop={args.pop_size}) ===")
    print(f"Comparing prepass=OFF (baseline) vs prepass=ON on {len(SUBSET)} equations\n")
    ...
    out_path = HERE / "results" / "feynman_decompose_ab.md"
...
def write_report(pairs, args, runtime, off_tally, on_tally, moves, out_path):
    L = []
    L.append("# Feynman A/B: decomposition pre-pass OFF vs ON")
    L.append("")
    L.append("**Per-equation A/B of multiplicative-separability pre-pass on")
    ...
    L.append("")
    L.append(f"**Pre-pass config**: R² threshold = 0.99 for accepting a power-law fit.")
    L.append(f"Detected fits are converted to seed trees `C · ∏ x_i^{{a_i}}` and injected")
    L.append(f"into the initial population. Selection determines if seeds survive.")
    L.append("")
    ...
    L.append("")
    L.append("```")
    L.append(f"python benchmarks/run_feynman_decompose_ab.py --n_gens {args.n_gens} --pop_size {args.pop_size}")
    L.append("```")
