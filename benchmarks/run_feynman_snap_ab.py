...
Step 2 of the Feynman improvement plan: after `optimize_constants`
polishes fitted scalars, attempt to snap each one to a canonical
physical value (1/2, 1/(4·pi), sqrt(2), e, ...) within
`snap_constants_rel_tol`. Snap is gated on loss preservation — only
accepted when the snapped tree's loss is no worse than the polished
one's.

Hypothesis: a meaningful fraction of "partial" verdicts have the right
structural skeleton but constants that are 1-5% off the canonical
physical value (1/(4π), 1/2, etc.). Snapping closes the gap on those
without harming exact or failed cases.

Pass criteria (independent justification, per §6 discipline):
  - At least 2 equations move "partial" → "exact"
  - No more than 0 regressions on currently-exact equations (snap is
    safe-by-construction; any regression indicates a bug)

Reuses SUBSET from run_feynman_extended.py (30 equations).

Wall-clock: ~12 min (6 min per arm).
"""
...
def run_with_snap(name, sampler, snap: bool, n_gens: int, pop_size: int):
    """Run one equation with snap either enabled or disabled."""
    env, y = sampler()
    ...
    cfg = GPConfig(
        ...
        snap_constants_enabled=snap,
        snap_constants_rel_tol=0.005,
    )
    gp = GP(cfg)
    t0 = time.time()
    ...
    return dict(
        name=name, snap=snap, n_vars=len(feature_names),
...
def main(argv=None):
    ...
    print(f"=== Feynman snap A/B (n_gens={args.n_gens}, pop={args.pop_size}) ===")
    print(f"Comparing snap=OFF (baseline) vs snap=ON on {len(SUBSET)} equations\n")
    ...
    for i, (name, formula, sampler, expected) in enumerate(SUBSET):
        ...
        try:
            r_off = run_with_snap(name, sampler, snap=False,
    ...
    print(f"snap=OFF: {off_tally['exact']} exact / {off_tally['partial']} partial / {off_tally['failed']} failed")
    print(f"snap=ON:  {on_tally['exact']} exact / {on_tally['partial']} partial / {on_tally['failed']} failed")
    ...
    out_path = HERE / "results" / "feynman_snap_ab.md"
...
def write_report(pairs, args, runtime, off_tally, on_tally, moves, out_path):
    L = []
    L.append("# Feynman A/B: constant snapping OFF vs ON")
    L.append("")
    L.append("**Per-equation A/B of canonical-constant snapping on the")
    L.append("30-equation Feynman subset.**")
    L.append("")
    ...
    L.append("")
    L.append(f"**Snap config**: rel_tol=0.005 (0.5%), ~94 candidate values")
    L.append(f"(integers, small rationals, pi/e/sqrt forms).")
    L.append("")
    ...
    L.append("")
    L.append("| Snap | Exact (rel<0.01) | Partial (rel<0.20) | Failed |")
    L.append("|---|---|---|---|")
    ...
    if delta_exact > 0:
        L.append(f"**Snap=ON wins on exact-count by +{delta_exact}**.")
    elif delta_exact < 0:
        L.append(f"**Snap=ON loses on exact-count by {delta_exact}**. Snap is safe-by-construction so this indicates a bug; investigate.")
    else:
    ...
    L.append("## Discovered expressions (snap=ON arm)")
    L.append("")
    for _off, on in pairs:
        ...
        L.append("  ```")
        L.append(f"  {ascii_tree[:200]}{'...' if len(ascii_tree) > 200 else ''}")
        L.append("  ```")
        L.append("")
    ...
    L.append("")
    L.append("```")
    L.append(f"python benchmarks/run_feynman_snap_ab.py --n_gens {args.n_gens} --pop_size {args.pop_size}")
    L.append("```")
