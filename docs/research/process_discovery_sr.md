...
> **C5 GRADUATED to production (2026-05-29):** Moved from `tessera.experimental.counterfactual_eval` → `tessera.search.counterfactual_eval`. First basket conjecture to complete the experimental→production lifecycle. Public API re-exported from `tessera.search.__init__`. Pre-analysis: `docs/shipped/c5_counterfactual_eval_analysis.md` (updated with graduation note). Validation evidence: heat eq benchmark + Feynman cross-bench (both unchanged from validation date).
